...
import useTimer from "../components/game-components/timer/useTimer";
...
export default function useGameEvents(username, opponent) {
  ...
  const opponentStartedGame = useCallback((gameJson) => {
  ...
  function handleDiceRoll(turnJson) {
  ...
  function handleOpponentLeft(leavingUser) {
  ...
  async function handleUserSelect(index) {
    setIsStartingPlayer(true);
    if (!canPlay) return;
    setIsSelecting(true);
    const memoizedGame = {
  ...
  function handleOpponentSelect(json) {
    select(json);
  ...
  function handleGameOver(gameJson, isWinner) {
...
function getEndGamePoints(game, thisTurn) {
  let points = 1;
